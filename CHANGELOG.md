...
- **Vector search results are always returned in descending similarity order.** `fn::vector_search` orders inside the function, but the outer `SELECT *` wrapper did not reliably preserve that order for multi-result queries, so `POST /api/search` could return a weaker match first. Results are now re-sorted by similarity in the domain layer (#1306, #1301)
- **Anthropic connection test no longer reports retired Haiku ids as HTTP 403.** The status-code check matched "403" inside the date suffix of `claude-3-haiku-20240307`, turning a plain "model not found" into a permissions error. Status codes are now matched as standalone numbers for 401/403/429, and the registry's test model moved to a current Haiku id (#1303, #1282)
- **Context-length errors are no longer retried in background jobs.** A source whose content exceeds the model's context window used to be retried up to 15 times (about 25 minutes) before failing, because the error was classified like a transient provider outage. A dedicated `ContextLengthExceededError` now stops the retry loop immediately (#1275)
